...
export function signInWithGoogle(appName = 'BlueCare Hospital') {
  const url = buildGoogleUrl(appName);
  if (!url) {
    window.dispatchEvent(new CustomEvent('app-toast', { detail: 'Google sign-in is not configured.' }));
    return;
  }
...
export async function handleGoogleRedirect() {
  ...
  if (!error) try { window.close(); } catch { /* The main window remains signed in. */ }
}
